...
import { responseMessage, signUp_verification_mail, forgotPassword_mail } from "../../helper";
    ...
    try {
        ...
        if (!existingMail) {
            ...
            if (response) {
                ...
                if (mail_send) {
                    return res.status(200).send(new apiResponse(200, responseMessage?.addDataSuccessMail('userData'), { body }, {}))
                }
...
//     try {
//         let response: any = await userModel.findOne({ body: ObjectId(id), isActive: true })
...
//             let mail_send = await forgotPassword_mail({ email: response?.email, name: response?.name, password: random_password })
//             if (mail_send) {
//                 return res.status(200).send(new apiResponse(200, responseMessage?.addDataSuccess('email'), { body }, {}))
//             }
//             else { return res.status(403).send(new apiResponse(403, responseMessage?.addDataError, {}, {})) }
//         }
//     }
